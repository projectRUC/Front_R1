...
export default function DesignSprintPage({ sprintId, usuId }: Props) {
  const {
    sprint,
    fases,
    loading,
  ...
  useEffect(() => {
    ...
    if (sprint.status && ["mapeo", "boceto", "decidir", "prototipo"].includes(sprint.status)) {
  ...
  const handleEnviarMapeo = async () => {
    await registrarMapeo(
      { proyecto_problema: problema, proyecto_objective: objetivo, enfoque },
      archivosLocales
    );
    limpiarUploader();
  };
  ...
  const handleEnviarBoceto = async () => {
    ...
    if (usuId === undefined || usuId === null || isNaN(parsedUsuId) || parsedUsuId <= 0) {
      alert("Error: No se ha detectado un ID de usuario válido. Vuelve a iniciar sesión.");
      return;
    }
  ...
  const handleEnviarPrototipo = async () => {
    await registrarPrototipo(
      { nombre_prototipo: nombrePrototipo, descripcion: descripcionPrototipo },
      archivosLocales
    );
    limpiarUploader();
  };

  if (!sprint) return <p className="text-sm text-gray-500 p-6">Cargando ciclo de ideación...</p>;

  // Validaciones de estado para bloquear múltiples publicaciones
  const mapeoRegistrado = Boolean(sprint.mapeo?.proyecto_problema);
  const bocetoPublicado = sprint.bocetos?.some((b) => b.usu_id === Number(usuId));
  const prototipoPublicado = Boolean(
    sprint.prototipo?.nombre_prototipo || (sprint.prototipo?.archivos && sprint.prototipo.archivos.length > 0)
  );

  return (
    <div className="max-w-3xl mx-auto p-6">
      <h1 className="text-xl font-bold mb-1">Ciclo de Ideación — Design Sprint</h1>
      <p className="text-sm text-gray-500 mb-4">Equipo #{sprint.eq_id}</p>

      {error && <p className="text-sm text-red-600 mb-3">{error}</p>}

      <DesignSprintTimeline fases={fases} faseActiva={faseActiva} onSelect={handleCambiarFase} />

      <div className="mt-4 border rounded-xl p-4 bg-white">
        {/* ================= MAPEAR (LUNES) ================= */}
        {faseActiva === "mapeo" && (
          <div className="flex flex-col gap-3">
            <h3 className="font-semibold">Mapear — Lunes</h3>
            <input
            ...
            {!mapeoRegistrado ? (
              ...
              <p className="text-sm text-green-600 font-medium text-center mt-2">
                ✓ La fase Mapear ya fue registrada para este ciclo. Puedes continuar a la pestaña "Bocetar".
              </p>
            )}
          </div>
        )}
        ...
        {faseActiva === "boceto" && (
          <div className="flex flex-col gap-3">
            <h3 className="font-semibold">Bocetar — Martes</h3>
            ...
            {!bocetoPublicado ? (
              ...
              <p className="text-sm text-green-600 font-medium text-center py-2 bg-green-50 rounded-lg border border-green-200">
            ...
            <EvidenceCard
              ...
              archivosGuardados={sprint.bocetos?.flatMap((b) => b.archivos) || []}
              previasLocales={previasLocales}
            />
          </div>
        )}
        ...
        {faseActiva === "decidir" && (
          <DecidirPhase
            ...
            onVotarBoceto={async (bocetoId: string) => {
              await puntuarBoceto(bocetoId, Number(usuId), 1);
            }}
          />
        )}
        ...
        {faseActiva === "prototipo" && (
          <div className="flex flex-col gap-3">
            <h3 className="font-semibold">Prototipar — Jueves</h3>
